...
//        Add plugin invocations by calling addPluginInvocation(pluginId,context type, configuration)
        addPluginInvocation("org.ambientdynamix.contextplugins.browsingInfo","org.ambientdynamix.contextplugins.browsingInfo.myBrowsingInfo",null);
    ...
    public void invokeOnResponse(ContextResult event) {
        ...
        if (event.hasIContextInfo()) {
            ...
            if (nativeInfo instanceof IMyBrowsingInfo) {
                IMyBrowsingInfo info = (IMyBrowsingInfo) nativeInfo;
                Log.i(TAG, "Browsing Info: " + info.getBrowsingInfo());
            }
